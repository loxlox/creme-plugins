...
function edit_texonomy_field( $taxonomy ) {
	...
	if ( taxonomy_image_url( $taxonomy->term_id, NULL, TRUE ) == CREMEDIMENTA_DEFAULT_IMAGE ) :
		$image_text = "";
	else :
...
function taxonomy_image_url( $term_id = NULL, $size = NULL, $return_placeholder = FALSE ) {
    ...
    if ( $return_placeholder ) {
		return ( $taxonomy_image_url != '' ) ? $taxonomy_image_url : CREMEDIMENTA_DEFAULT_IMAGE;
    } else {
...
function taxonomy_image( $term_id = NULL, $size = 'full' ) {
	...
	if( !empty( $taxonomy_image_url ) ) {
		...
		$taxonomy_image = '<img src="' . esc_url( CREMEDIMENTA_DEFAULT_IMAGE ) . '" class="attachment-full size-full" alt="' . trim( strip_tags( $title ) ) . '" />';
